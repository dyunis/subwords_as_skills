...
class Actor(BasePolicy):
    """
    Actor network (policy) for SAC-discrete.
    ...
    def __init__(
        ...
        latent_pi_net = create_mlp(features_dim, action_dim, net_arch, activation_fn)
    ...
    def get_action_distrib(self, obs: th.Tensor) -> th.Tensor:
        logits = self.get_action_dist_params(obs)
...
class DiscreteCritic(BaseModel):
    """
    Critic network for SAC-discrete.
    It represents the action-state value function (Q-value function).
    Like the A2C/PPO critics, this one represents the Q-value
    and takes the continuous action as input. It is concatenated with the state
    and then fed to the network which outputs a vector Q(s) with values for every a.
...
MlpPolicy = SACDiscretePolicy


class CnnPolicy(SACDiscretePolicy):
    """
    Policy class (with both actor and critic) for SAC-discrete.
...
class MultiInputPolicy(SACDiscretePolicy):
    """
    Policy class (with both actor and critic) for SAC-discrete.
